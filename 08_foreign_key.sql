/*
[FOREIGN KEY]
1. A foreign key is a column that creates a link between two tables
2. It ensures that the value in one table must match a value in another table
*/
USE startersql;

-- EXAMPLE OF FOREGIN KEY
CREATE TABLE addresses (
 ...
 state VARCHAR(100),
 pincode VARCHAR(10),
 FOREIGN KEY (user_id) REFERENCES users(id) -- user_id is foregin key which is linked to the column id in users table which is a primary key
);
